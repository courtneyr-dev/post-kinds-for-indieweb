...
Create your first post kind and learn how the pieces fit together. This assumes the plugin is [installed and activated](installation.md).
...
2. Open the block inserter and find the **Post Kinds for IndieWeb** category, or type `/listen`. Insert a **Listen Card**. (Screenshot: [block inserter](assets/screenshots/editor-block-inserter.png).)
...
1. **Add API keys for richer search.** MusicBrainz and Open Library work without keys, but movie/TV search (TMDB), games (RAWG, BoardGameGeek), and others need keys. Go to **Reactions → API Connections**, where each service links to its sign-up page and has a "Test connection" style check. See [Settings](settings.md#api-connections-page-reactions--api-connections).
2. **Review the General tab.** **Reactions → Settings** controls the default category for kind posts, default post status, microformats, syndication, and post-format syncing. See [Settings](settings.md).
3. **Set your check-in privacy default.** If you plan to post check-ins, decide up front how much location detail to publish. The Checkin tab offers public / approximate / private defaults and coordinate handling. See [Privacy and data](privacy-and-data.md).
4. **Import your history.** **Reactions → Import** pulls in listening, watching, and reading history from connected services; large imports run in the background. See [Common tasks](common-tasks.md).
...
- **"Media search finds nothing."** Usually missing API keys or a server that can't make outbound HTTPS requests. See [Troubleshooting](troubleshooting.md#media-search-returns-no-results).
...
- **"I posted from my phone app and nothing happened."** Micropub posting requires the separate Micropub plugin (plus IndieAuth). See [Troubleshooting](troubleshooting.md#posts-from-a-micropub-app-dont-arrive).
- **"My cards looked wrong after an update."** Versions 1.4.1–1.4.3 fixed a run of card layout regressions (scattered editor layout, stripped padding and shadow on the front end). If cards look off, update to the latest version first.
