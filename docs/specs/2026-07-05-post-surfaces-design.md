...
- **Filter `pkiw_stream_kinds`** — `apply_filters( 'pkiw_stream_kinds', [] )` → array of kind slugs treated as ephemeral. **Default empty** (no behavior change for existing installs). readme documents a recommended set. This is the "easy for others to decide similarly" seam.
- **Override meta `pkiw_promote`** — boolean, `register_post_meta` on kind-bearing post types, `show_in_rest => true`, `single => true`, edit-gated `auth_callback`. Non-underscore key (REST + block-binding safe). Meaning: "treat this post as main regardless of kind."
- **Derived meta `_pkiw_surface`** — `'stream' | 'main'`, read-only/protected, recomputed on `save_post`: `stream` if the post has a kind in `pkiw_stream_kinds` **and** `pkiw_promote` is not set; else `main`. Stored so sites can query it with a single meta clause.
- **Helper + filter** — `\PostKindsForIndieWeb\get_post_surface( $post ): string` and a `pkiw_post_surface` filter for full programmatic override.
- **Editor control** — a `PluginDocumentSettingPanel` toggle ("Promote to main archive", label filterable) bound to `pkiw_promote`, shown only for kind-bearing types. (JS → rebuild `build/`.)
- **Micropub mapping** — the Micropub intake that creates the post maps an incoming property `pkiw-promote` → `pkiw_promote` meta, so Micropub clients (Outpost) can set the override at creation. *Integration point: confirm which endpoint actually processes Outpost posts on the site (PKIW content builder vs IndieBlocks vs Outpost endpoint) and add the mapping there.*
- **WP-CLI** — `wp postkind surfaces backfill` recomputes `_pkiw_surface` for all posts.
...
- mu-plugin sets `add_filter( 'pkiw_stream_kinds', fn() => [ 'checkin','eat','drink','listen','jam','mood','play','acquisition' ] )`.
- `pre_get_posts` gated to the Blog main query only — `! is_admin() && $q->is_main_query() && $q->is_home() && ! $q->is_feed()` — excludes `_pkiw_surface = stream` (`relation OR`: `_pkiw_surface NOT EXISTS` or `!= stream`, so missing = main).
- Child theme: **Stream** page via a filtered Query Loop (`_pkiw_surface = stream`), ~20/page, secondary query only; add **Stream** to primary nav.
...
- On submit it includes Micropub property `pkiw-promote: true` when on.
...
1. **PKIW** (meta, `_pkiw_surface`, filter, editor toggle, Micropub mapping, CLI, tests) — everything else depends on it.
...
`pkiw_stream_kinds` (filter) · `pkiw_promote` (public override meta) · `_pkiw_surface` (derived, protected) · `pkiw_post_surface` (filter) · `pkiw-promote` (Micropub property).
...
- Posts created outside `save_post` won't have `_pkiw_surface` → default to `main`; `wp postkind surfaces backfill` is the catch-all. Micropub creates fire `save_post`, so Outpost posts are fine.
- Kind changed post-publish → recompute on save.
- Existing PKIW installs: empty default `pkiw_stream_kinds` → nothing changes until a site opts in.
