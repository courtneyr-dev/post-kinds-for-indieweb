...
class OwnTracks_Checkin_Sync extends Checkin_Sync_Base {
	...
	private function create_checkin_from_location( array $payload ) {
		...
		// Build title.
		$title = $this->build_location_checkin_title( $poi, $address, $lat, $lon );
		...
		if ( ! $post_id ) {
			return new \WP_Error( 'insert_failed', __( 'Failed to create post.', 'post-kinds-for-indieweb-in-block-themes' ) );
		}
		...
		update_post_meta( $post_id, '_pkiw_checkin_name', $this->get_checkin_name( $poi, $address ) );
		...
		// Address components.
		if ( ! empty( $address ) ) {
			$this->save_address_meta( $post_id, $address );
		}
	...
	/**
	 * Build a checkin title from location data.
	 *
	 * @param string               $poi     Point of interest name.
	 * @param array<string, mixed> $address Reverse-geocoded address.
	 * @param float                $lat     Latitude.
	 * @param float                $lon     Longitude.
	 * @return string
	 */
	private function build_location_checkin_title( string $poi, array $address, float $lat, float $lon ): string {
		if ( '' !== $poi ) {
			return $poi;
		}
	...
	/**
	 * Get the saved checkin name.
	 *
	 * @param string               $poi     Point of interest name.
	 * @param array<string, mixed> $address Reverse-geocoded address.
	 * @return string
	 */
	private function get_checkin_name( string $poi, array $address ): string {
		if ( '' !== $poi ) {
			return $poi;
		}
	...
	/**
	 * Save address fields onto the checkin post.
	 *
	 * @param int                  $post_id Post ID.
	 * @param array<string, mixed> $address Address data.
	 * @return void
	 */
	private function save_address_meta( int $post_id, array $address ): void {
		$address_parts = isset( $address['address'] ) && is_array( $address['address'] ) ? $address['address'] : [];

		if ( ! empty( $address_parts['road'] ) ) {
			update_post_meta( $post_id, '_pkiw_checkin_street', $address_parts['road'] );
		}

		$locality = $this->get_address_locality( $address_parts );
		if ( '' !== $locality ) {
			update_post_meta( $post_id, '_pkiw_checkin_locality', $locality );
		}

		if ( ! empty( $address_parts['state'] ) ) {
			update_post_meta( $post_id, '_pkiw_checkin_region', $address_parts['state'] );
		}

		if ( ! empty( $address_parts['country'] ) ) {
			update_post_meta( $post_id, '_pkiw_checkin_country', $address_parts['country'] );
		}

		if ( ! empty( $address_parts['postcode'] ) ) {
			update_post_meta( $post_id, '_pkiw_checkin_postal_code', $address_parts['postcode'] );
		}
	}

	/**
	 * Get the best locality name from an address payload.
	 *
	 * @param array<string, mixed> $address_parts Address parts.
	 * @return string
	 */
	private function get_address_locality( array $address_parts ): string {
		foreach ( [ 'city', 'town', 'village' ] as $key ) {
			if ( ! empty( $address_parts[ $key ] ) ) {
				return (string) $address_parts[ $key ];
			}
		}
