...
class Untappd_Checkin_Sync extends Checkin_Sync_Base {
	...
	public function handle_oauth_callback( string $code ): bool {
		...
		// Get user info.
		$user_info = $this->api_request( '/user/info', [], $body['response']['access_token'] );
	 ...
	 */
	private function api_request( string $endpoint, array $params = [], ?string $access_token = null, string $method = 'GET' ) {
		$credentials = $this->get_credentials();
