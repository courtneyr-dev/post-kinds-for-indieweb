...
<article <?php echo $pkiw_wrapper_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
	...
	<div class="pk-body">
		<span class="pk-kindlabel"><?php echo esc_html( get_kind_label( __( 'Wish', 'post-kinds-for-indieweb-in-block-themes' ), 'wish', 'wish-card' ) ); ?></span>

		<div class="pk-caption">
			<?php if ( $pkiw_title ) : ?>
				<h2 class="pk-title p-name">
					<?php if ( $pkiw_url ) : ?>
						<a class="u-url u-wish-of" href="<?php echo esc_url( $pkiw_url ); ?>" target="_blank" rel="<?php echo esc_attr( $pkiw_link_rel ); ?>"<?php echo \PKIW\pkiw_new_tab_label_attr( $pkiw_title ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>><?php echo esc_html( $pkiw_title ); ?></a>
					<?php else : ?>
