	...
	private function assertion_exceptions(): array {
		return [
			...
			'post-kinds-indieweb/event-card'        => [
				...
				'eventEnd'        => 'rendered as a dt-end <data> only after strtotime() parses it, same gate as eventStart',
				'calendarSource'  => 'calendar plugin selector consumed by the render-time lookup; the sample matches no active source so nothing is fetched and the slug is never echoed',
				'calendarEventId' => 'calendar event lookup key for the render-time fetch, never echoed (the single-digit sample can match markup by coincidence, so the exception keeps the gate honest)',
			],
