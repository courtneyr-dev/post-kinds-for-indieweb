/**
 * Reply Card Block - Edit Component
 *
...
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: 'reply-card-block pk-card k-reply',
	} );
	...
	// Set post kind to "reply" when block is inserted
	useEffect( () => {
		if ( ! currentKind ) {
			wp.apiFetch( { path: '/wp/v2/kind?slug=reply' } )
				.then( ( terms ) => {
	...
	useEffect( () => {
		const metaUpdates = {};
		if ( title !== undefined ) {
			metaUpdates._postkind_reply_title = title || '';
		}
		if ( url !== undefined ) {
			metaUpdates._postkind_reply_url = url || '';
		}
		if ( author !== undefined ) {
			metaUpdates._postkind_reply_author = author || '';
		}
		if ( image !== undefined ) {
			metaUpdates._postkind_reply_image = image || '';
		}
	...
	const handleImageSelect = ( media ) => {
		setAttributes( {
			...
			imageAlt:
				media.alt ||
				title ||
				__( 'Reply context image', 'post-kinds-for-indieweb' ),
		} );
	};
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={ __( 'Reply Details', 'post-kinds-for-indieweb' ) }
					initialOpen={ true }
				>
					<TextControl
				...
				<div className="post-kinds-card">
					...
					<div className="post-kinds-card__content">
						<span className="post-kinds-card__badge">
							&#8618;{ ' ' }
							{ __( 'In reply to', 'post-kinds-for-indieweb' ) }
						</span>
