			...
			<?php if ( $pkiw_book_title ) : ?>
				<h2 class="pk-title p-name">
					<?php if ( $pkiw_book_url ) : ?>
						<a class="u-url" href="<?php echo esc_url( $pkiw_book_url ); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html( $pkiw_book_title ); ?><?php echo \PKIW\pkiw_new_tab_hint(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a>
					<?php else : ?>
