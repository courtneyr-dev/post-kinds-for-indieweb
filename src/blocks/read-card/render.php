...
<article <?php echo $pkiw_wrapper_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
	...
	<div class="pk-body">
		<span class="pk-kindlabel"><?php echo esc_html( get_kind_label( __( 'Read', 'post-kinds-for-indieweb-in-block-themes' ), 'read', 'read-card' ) ); ?></span>

		<div class="pk-caption">
			<?php if ( $pkiw_book_title ) : ?>
				<h2 class="pk-title p-name">
					<?php if ( $pkiw_book_url ) : ?>
						<a class="u-url" href="<?php echo esc_url( $pkiw_book_url ); ?>" target="_blank" rel="noopener noreferrer"<?php echo \PKIW\pkiw_new_tab_label_attr( $pkiw_book_title ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>><?php echo esc_html( $pkiw_book_title ); ?></a>
					<?php else : ?>
