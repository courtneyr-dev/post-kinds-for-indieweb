/**
 * Repost Card Block - Edit Component
 *
...
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: 'repost-card-block pk-card k-repost',
	} );
	...
	// Set post kind to "repost" when block is inserted
	useEffect( () => {
		if ( ! currentKind ) {
			wp.apiFetch( { path: '/wp/v2/kind?slug=repost' } )
				.then( ( terms ) => {
	...
	useEffect( () => {
		const metaUpdates = {};
		if ( title !== undefined ) {
			metaUpdates._postkind_repost_title = title || '';
		}
		if ( url !== undefined ) {
			metaUpdates._postkind_repost_url = url || '';
		}
		if ( author !== undefined ) {
			metaUpdates._postkind_repost_author = author || '';
		}
		if ( image !== undefined ) {
			metaUpdates._postkind_repost_image = image || '';
		}
	...
	const handleImageSelect = ( media ) => {
		setAttributes( {
			...
			imageAlt:
				media.alt ||
				title ||
				__( 'Reposted content image', 'post-kinds-for-indieweb' ),
		} );
	};
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={ __( 'Repost Details', 'post-kinds-for-indieweb' ) }
					initialOpen={ true }
				>
					<TextControl
				...
				<div className="post-kinds-card">
					...
					<div className="post-kinds-card__content">
						<span className="post-kinds-card__badge">
							&#8635;{ ' ' }
							{ __( 'Reposted', 'post-kinds-for-indieweb' ) }
						</span>
