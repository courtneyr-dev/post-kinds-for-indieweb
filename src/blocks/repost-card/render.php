 ...
 *
 * Renders the repost card in the two-layer pk-card system: plugin owns
 * structure (badge → label → title → sub → note → meta), theme owns paint
 * via --pk-* custom properties. The linked title carries the `u-repost-of`
 * microformats2 class so parsers see the repost target.
 *
 * @package PKIW
...
<article <?php echo $pkiw_wrapper_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
	...
	<div class="pk-body">
		<span class="pk-kindlabel"><?php echo esc_html( get_kind_label( __( 'Repost', 'post-kinds-for-indieweb-in-block-themes' ), 'repost', 'repost-card' ) ); ?></span>
		...
		<?php if ( $pkiw_reposted_at ) : ?>
			<div class="pk-meta">
				<time class="dt-published" datetime="<?php echo esc_attr( gmdate( 'c', strtotime( $pkiw_reposted_at ) ) ); ?>"><?php echo esc_html( wp_date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $pkiw_reposted_at ) ) ); ?></time>
			</div>
		<?php endif; ?>
	</div>
</article>
