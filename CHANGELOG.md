...
- `Webhook_Handler::validate_token()`, the per-service token check that runs for endpoints added through the `pkiw_webhook_endpoints` filter, no longer reads the token from the `?token=` query string or the request body; only the `X-Webhook-Token` and `Authorization: Bearer` headers are read. The five built-in routes are unchanged: they authorize in their `permission_callback` and pass `$authenticated = true`, which skips this check. Plex keeps its documented query-string token in `verify_plex_webhook_token()`.
...
- The admin media lookup, quick-search, and import-preview panels built result markup by string-concatenating provider fields (title, artist, year, image URL, error messages) straight into `.html()`, so a provider result containing markup (e.g. `<img src=x onerror=...>`) executed in the admin's browser. `admin/js/admin.js` now builds these panels with `document.createElement`/`.text()` via a new `admin/js/lookup-render.js` module (loaded as `PKIWLookupRender`), and every API client's `normalize_result()` now runs its string fields through `wp_strip_all_tags()` (image/cover/poster URLs through `esc_url_raw()` instead) via a shared `API_Base::sanitize_normalized_result()`.

## [1.8.4] - 2026-09-14

### Fixed

- The Checkin Card's location privacy control and the Default Location Privacy setting still described Approximate as city level ("Only city/region will be shown. Coordinates are stored but not displayed."), and the editor preview hid the street address and zoomed the map out for Approximate, while check-ins now show the exact location unless private. The option is now labeled Approximate, a warning says check-ins show the exact location the same as Public, the preview shows the address and exact map unless private, and the settings and docs describe the venue rule.
...
- The listen, watch, read, play, eat and drink cards' `.p-rating` star rating held only `aria-hidden` SVG stars, so mf2 parsers read `rating` as an empty string instead of the numeric value. `.pk-stars` no longer carries the `p-rating` class; each card now emits a sibling, empty `<data class="p-rating" value="N" hidden></data>` after the visible stars — the same pattern `class-microformats.php`'s review case already uses — so `rating` parses correctly without ever putting the digit in an element's text content (an earlier fix put the digit in a hidden child's text, which parsed correctly but leaked into `strip_tags`-based plain-text extraction that doesn't respect `hidden`). Visible star output, the `role="img"` element and its `aria-label` accessible name, and CSS are unchanged; a card with no rating still emits no `.pk-stars`/`.p-rating` markup at all.
- The eat card's `restaurant` name and its `_pkiw_eat_restaurant` REST meta still showed on private eat posts after the R-03 location-privacy fix: the card printed the restaurant as `p-location h-card > p-name` with no gate, and the REST redaction map didn't list the key. Both now follow the name tier — approximate keeps the restaurant name, private hides it from anyone who cannot edit the post.
- Location privacy (R-03) was binary — a post was treated as either "public" or "not public" — so "approximate" leaked precise detail in some places and "private" didn't fully hide venue name/locality/region/country in others. `Meta_Fields::get_visible_location_fields( $post_id )` is now the single source of truth for what's visible per field (name, locality, region, country, street, postal code, coordinates, map, url, osm id, venue id), and cards, block bindings, REST meta redaction and the check-in location helper all call it instead of their own ad hoc checks. Approximate now consistently shows only venue/restaurant name, locality, region and country; private shows nothing to anyone who cannot edit the post. Concretely: the eat and drink cards rendered their venue/restaurant URL, street address and coordinates unconditionally, with no privacy gate at all; the checkin card's map still leaked a real (if bbox-widened) marker position on approximate, and its OpenStreetMap/Foursquare ids and venue link weren't gated by anything; REST responses stripped precise fields (address, coordinates, url, OSM id) but never stripped name/locality/region/country, so those leaked through on private posts; the `post-kinds-indieweb/kind-meta` block binding for a check-in's venue name wasn't gated at all. The map is omitted entirely for approximate check-ins rather than shown with a coarsened position — a widened-bbox map still marks a point close to the real one. A new `pkiw_get_visible_location_fields( $post_id )` global function (guarded by `function_exists()`) exposes the same contract to themes. Anyone who can edit the post continues to see full values, as before.
- The `post-kinds/kind-meta` block bindings source, the checkin helper functions, and every card block's "assign kind on insert" effect read and wrote a taxonomy called `indieblocks_kind`, which nothing registers — the plugin's real, registered taxonomy is `kind` (`PKIW\Taxonomy::TAXONOMY`, REST base `kind`). A theme part binding a listen post's rating or URL rendered empty, check-in queries and archive helpers found nothing, and inserting a card block silently failed to assign its kind term. The Check-ins Feed block's REST fallback also queried `indieblocks_kind` as a slug instead of resolving the `kind` term ID first, which the REST taxonomy filter requires. Fixed by using `Taxonomy::TAXONOMY` in PHP and the `kind` REST attribute in JS throughout; no released version ever registered `indieblocks_kind`, so no migration or compatibility shim is needed.
...
- `_pkiw_surface` is also recomputed when the `pkiw_promote` flag is added, changed or removed as a plain meta write (REST `meta`, Quick Edit, WP-CLI), which previously left the stored surface one save behind the toggle.
- The webhook REST callbacks (`/webhook/listenbrainz`, `trakt`, `plex`, `jellyfin`, `generic`) called handler methods that did not exist and answered every authenticated POST with a 500; they now dispatch through `Webhook_Handler::handle_request()`.
- REST responses honor Simple Location's `geo_public` for readers who cannot edit the post: private (0) blanks coordinates and address in `meta` and in IndieBlocks' `indieblocks_location` field, protected (2) blanks coordinates only. Stored values are untouched.
- Every Stream card and singular entry carries a hidden `p-author h-card` (name, URL, photo) when it has no author, so parsers never fall back to page-level guesses; the identity is filterable with `pkiw_entry_author`.
