...
- Nine new post kinds, completing coverage of every kind on [indieweb.org/posts](https://indieweb.org/posts#Types_of_Posts) except the single-site experimental tier: audio, quotation, follow, presentation, collection, comics, issue, exercise, and chicken. All register as default taxonomy terms (with kind-selector icons), and the ones with established Micropub properties are auto-detected on ingest: `quotation-of` (checked after `in-reply-to` — a quotation with commentary is a reply per the wiki), `audio` (Micropub's reserved media property, checked before `photo` so cover art doesn't reclassify an audio post), and `follow-of`, which previously kept the `note` default because no `follow` term existed. Content-less quotation and audio posts get generated markup carrying the documented microformats (`u-quotation-of h-cite`, `u-audio`) instead of dying as phantom posts. Existing installs gain the new terms automatically — the term backfill now keys on a fingerprint of the default-kind set instead of the plugin version, so added kinds appear without a version bump.
