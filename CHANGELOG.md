...
- The listen, watch, read, play, eat and drink cards' `.p-rating` star rating held only `aria-hidden` SVG stars, so mf2 parsers read `rating` as an empty string instead of the numeric value. Each card now carries a hidden `<data class="value">` (text content matching its `value` attribute — this `mf2/mf2` version's value-class-pattern implementation reads text content, not the `value` attribute, for a class="value" child) inside `.p-rating`, so `rating` parses correctly. Visible star output, the `role="img"` element and its `aria-label` accessible name, and CSS are unchanged; a card with no rating still emits no `.p-rating` markup at all.
